...
if [[ "$DockerHealth" == "dead" ]]; then
    echo -e "Subject: CRITICAL ALERT - Docker is dead on $(hostname)\n\nDocker service is not running.\n\nStatus: $DockerHealth\nDocker current Version: $DockerVersion" | msmtp manojdevopstest@gmail.com
    ...
    echo -e "Docker service is restarted... on $(hostname).\n\nStatus: $NewHealth \nDocker current Version: $DockerVersion"\
     | mail -s "INFO ALERT - RESTORED THE DOCKER HEALTH ON $(hostname)" manojdevopstest@gmail.com
    echo "Aler is sent"
fi
